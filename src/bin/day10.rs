...
fn count_up(head: (usize, usize), data: &[Vec<u32>], count: &mut HashSet<(usize, usize)>) -> usize {
    ...
    for dir in [(-1, 0), (0, 1), (1, 0), (0, -1)] {
        let next_y = head.0 as i64 + dir.0;
        if next_y < 0 || next_y >= data.len() as i64 {
            continue;
        }
        let next_x = head.1 as i64 + dir.1;
        if next_x < 0 || next_x >= data[0].len() as i64 {
            continue;
        }
        if data[next_y as usize][next_x as usize] == curr + 1 {
            result += count_up((next_y as usize, next_x as usize), data, count);
        }
    }
    result
}
