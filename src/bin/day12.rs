...
fn perimeter(region: &HashSet<(usize, usize)>, limit: (usize, usize)) -> usize {
    region
        .iter()
        .map(|loc| {
            let mut result = 0;
...
fn discount(region: &HashSet<(usize, usize)>, limit: (usize, usize)) -> usize {
    region
        .iter()
        .map(|loc| {
            let mut result = 0;
...
fn count(data: &[Vec<char>], step: &Step) -> usize {
    ...
    regions
        ...
        .map(|region| {
            region.len()
                * match step {
                    Step::First => perimeter(region, (data.len(), data[0].len())),
                    Step::Second => discount(region, (data.len(), data[0].len())),
                }
        })
        .sum()
}
